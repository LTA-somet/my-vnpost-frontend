...
export default () => {
    ...
    const libraryTypeId: number = +location.pathname.replace('/news/news-general/', '');
                        ...
                        <div key={element.libraryInfoId}>
                            <table id='table-design' onClick={() => viewDetail(element)} >
                                <tr style={{ border: "2px" }}>
                                    ...
                                    <td rowSpan={2} style={{ textAlign: "center" }}>{element.createdDate} | <span><EyeOutlined title='Xem chi tiết' style={{ marginTop: '5px' }} /> {element.countView}</span></td>
                                </tr>

                                <tr>
                                    {element.loadType === "0" ? <td style={{ width: '80%' }}> {element.focusNews ? element.focusNews : <span><i>Không có dữ liệu hiển thị</i></span>}</td>
                                        : <td style={{ width: '80%' }}>{element.fileName ? element.fileName : <span><i>Không có dữ liệu hiển thị</i></span>}</td>}
