...
export default () => {
    ...
    const libraryTypeId: number = +location.pathname.replace('/manage/library-manager/news/', '');
    ...
    useEffect(() => {
        ...
        let dataFilter = dataSource.filter(c => c.libraryTypeId === libraryTypeId) || [];
        if (titleFilter) {
                    ...
                    dataTableFilter.map((element: LibraryInfoDto) => (
                        <div key={element.libraryInfoId}>
                            <table id='table-design' onClick={() => viewDetail(element)} >
                                <tr style={{ border: "2px" }}>
                                    <td style={{ width: '80%', fontWeight: 'bold' }}>{element.title}</td>
