...
const { Option } = Select;
const EditVasConfiguration: React.FC<Props> = (props: Props) => {
    ...
    const actionVasPropShow = (value: boolean, record: VasPropsDto): React.ReactNode => {
        return <Space key={record.propCode}>
            <Checkbox defaultChecked={value} onChange={e => onChangeShow(record, e.target.checked)} />
        </Space>
    }
    ...
    return (
        <div>
            <Modal
                title={<div style={{ fontSize: '16px' }}>{'Cấu hình thuộc tính giá trị gia tăng'}</div>}
...
export default EditVasConfiguration;