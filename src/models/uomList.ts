import { McasUomDto, UomApi } from '@/services/client';
import { addToDataSource, deleteFromDataSource, updateToDataSource } from '@/utils/dataUtil';
import { useCallback, useState } from 'react';

const uomApi = new UomApi();
export default () => {
  const [dataSource, setDataSource] = useState<McasUomDto[]>([]);
  ...
  const createRecord = useCallback(
    (record: McasUomDto, callback?: (success: boolean) => void) => {
      setSaving(true);
      uomApi
        .createUom(record)
        .then((resp) => {
  ...
  const updateRecord = useCallback(
    (id: string, record: McasUomDto, callback?: (success: boolean) => void) => {
      setSaving(true);
      uomApi
        .updateUom(id, record)
        .then((resp) => {
          ...
          if (resp.status === 201) {
            setDataSource(updateToDataSource(dataSource, resp.data, 'uomId'));
          }
        })
        .finally(() => setSaving(false));
    },
