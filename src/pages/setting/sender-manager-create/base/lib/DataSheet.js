...
export default class DataSheet extends PureComponent {
    ...
    pageClick(e) {
        ...
        try {
            ...
            if(!clickedCellId) { //not clicked table cell but clicked value viewer or value editor
                clickedCellId = e.target.parentNode.id
            }

            if(clickedCellId && clickedCellId.includes('table_') && clickedCellId.includes('_row_') && clickedCellId.includes('_col_')){
                const clickedTableId = clickedCellId.substring(0,clickedCellId.indexOf("_row_"));
                if(clickedTableId !== currentTableId){
    ...
    handleKey(e) {
        ...
        if (noCellsSelected || (ctrlKeyPressed && (keyCode == 67 || keyCode == 86 || keyCode == 88) )) { //ctrl + C, ctrl + V, ctrl + X
            return true;
        }
    ...
    scrollToCell(row, col) {
        ...
        const farestContainer = this.dgDom && this.farestByClass(this.dgDom, 'DIV', 'horizontal-scroll')

        // const element = container && document.getElementById(container.id + '_row_' + row + '_col_' + col)
        const closestContainer = this.dgDom && this.closestByClass(this.dgDom, 'DIV', 'horizontal-scroll')
        const element = closestContainer && document.getElementById(closestContainer.id + '_row_' + row + '_col_' + col)
        if (!element) {
        ...
        if (elementRect.left < conatinerRect.left) { //element was hidden in the LEFT SIDE of the container
            if (col == 0) {
        ...
        let leftPixel = (elementRect.left + elementRect.width) - (conatinerRect.left + conatinerRect.width);
        if (leftPixel > 0) {//element was hidden in the RIGHT SIZE of the container 
            //-> scroll to the LEFT to show the completed element
            farestContainer.scrollLeft = farestContainer.scrollLeft + leftPixel + 5;
        }

        if (elementRect.top > window.innerHeight) { //element was hidden in the BOTTOM
            //-> scroll UP
    ...
    render() {
        ...
        return (
            <div className={topClass} id={'table_' + tableId} style={{ width: '100%', border: '0px' }}>
                <span
                    ref={r => {
                        this.dgDom = r;
                    }}
                ...
                >
                    ...
                    >
                        {data.map((row, i) => (
                            <RowRenderer key={keyFn ? keyFn(i) : i} row={i} cells={row} tableId={tableId}>
                                {row.map((cell, j) => {
